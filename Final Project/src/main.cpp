...
<body>
  ...
  <form action="/get">
    ...
    Recipient Email add<input type="text" step="0.1" name="input2" value="%THRESHOLD%" required><br>
...
void loop()
{
  ...
  if ((millis() - lastTime) > timerDelay)
  {
    ...
    if ((temperatureC.toFloat() >= inputMessage3.toFloat())) // check for mail sending temperature threshold  && (sendMailCheck == true)
    {
      if (EmailMsgCount == 0)
      {
        smtp.debug(1);
